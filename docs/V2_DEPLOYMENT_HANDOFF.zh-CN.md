...
- 已将 `master` 的 `5c3709f` 既有队列、爬虫、布局和 HTTPS 修复合入开发分支，保留连续分数、V2 导航和相对跳转；没有修改主分支或部署。
...
- Emby 初次分页可恢复；后续采用轻量完整清单核对和 Etag 增量元数据请求，服务器不提供 Etag 时安全回退完整页。完整清单用于删除检测，Web/CLI 共享同步租约防止交错发布。媒体离线判断来自 Emby 返回信息，未实测用户服务器。已有文件不移动、不删除、不改元数据/播放历史。
- JavDB 本轮实测 403，真实字段适配未启用，详见 `V2_SOURCE_EVIDENCE.zh-CN.md`。
- Python 3.10 与原 Python 3.7.4 / NumPy 1.17.1 / scikit-learn 0.21.3 隔离容器测试均已通过。生产 Dockerfile 保留旧依赖并增加 fork 源码路径断言；完整生产镜像构建、旧爬虫运行及 NAS 实际挂载仍须实测。
